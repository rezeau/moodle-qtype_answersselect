...
function xmldb_qtype_answersselect_upgrade($oldversion) {
    ...
    if ($oldversion < 2020031600) {
        ...
        if (!get_config('qtype_answersselect', 'combined2020031600upgradealreadyrun')) {
            $DB->execute("
                    UPDATE {question_answersselect}
                     ...
                     WHERE questionid IN (
                            SELECT child.id
                             ...
                             WHERE combined.qtype = 'combined'
                               AND child.qtype = 'answersselect'
                           )
                ");
        }

        upgrade_plugin_savepoint(true, 2020031600, 'qtype', 'answersselect');
    }
    ...
    if ($oldversion < $newversion) {
        ...
        // Define field id to be added to question_answersselect.
        ...
        // answersselect savepoint reached.
        upgrade_plugin_savepoint(true, $newversion, 'qtype', 'answersselect');
    }
    return true;
}
